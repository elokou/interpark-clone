(() => {
  ...
  function detectElements() {
    ...
    const seatSearch = globalThis.STASeatSearchLoop.scan({ seats, quantity: settings.quantity });
    ...
    return { areaButtons: recommendedAreas, seats, seatSearch, selectedSeatCount, nextButtons, seatAnalysis, areaAnalysis, map, iframes, interpark, dom: { areas: areaButtons.slice(0, 6).map(describe), seats: seats.slice(0, 6).map(describe), next: nextButtons.slice(0, 4).map(describe) } };
  }
  ...
  function statusPayload(elements = detectElements()) {
    return {
      ...
      recognized: { areas: elements.areaButtons.length, seats: elements.seatSearch.candidates.length, next: elements.nextButtons.length },
  ...
  async function verifySeatClick(elements) {
    ...
    if (Date.now() - pendingSeatClick.startedAt >= 2500) {
      ...
      await appendLog(`座位点击未确认：${pendingSeatClick.labels.join(', ')}；5秒后重新扫描当前区域`);
  ...
  async function assist(elements) {
    ...
    const hasAvailableSeats = elements.seatSearch.candidates.length > 0;
    const decision = areaRotation.next({ hasAvailableSeats });
    ...
    if (hasAvailableSeats && settings.autoSelectSeats) {
      const beforeState = globalThis.STASeatClickEngine.selectionState();
      const clickedSeats = globalThis.STASeatClickEngine.selectRecommended(elements.seatSearch.recommended, settings.quantity);
      if (clickedSeats.length) {
        executionState.currentSeat = clickedSeats.join(', ');
        executionState.clickCount += clickedSeats.length;
        pendingSeatClick = { labels: clickedSeats, beforeState, startedAt: Date.now() };
