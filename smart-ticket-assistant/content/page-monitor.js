(() => {
  ...
  function detectElements() {
    ...
    return { areaButtons: recommendedAreas, seats, seatSearch, canvasSearch, selectedSeatCount, nextButtons, seatAnalysis, areaAnalysis, map, iframes, interpark, dom: { areas: areaButtons.slice(0, 6).map(describe), seats: seats.slice(0, 6).map(describe), next: nextButtons.slice(0, 4).map(describe) } };
  }
  ...
  async function assist(elements) {
    ...
    const hasAvailableSeats = elements.seatSearch.candidates.length > 0 || elements.canvasSearch.candidates.length > 0;
    ...
    if (hasAvailableSeats && settings.autoSelectSeats && !forceAreaRotation) {
      ...
      if (clickedSeats.length) {
        ...
        pendingSeatClick = { labels: clickedSeats, beforeState, startedAt: Date.now(), canvas: false };
