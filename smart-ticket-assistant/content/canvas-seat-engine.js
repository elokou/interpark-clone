(() => {
  ...
  const isAvailableGreen = ([r, g, b]) => g >= 90 && g > r * 1.2 && g > b * 1.15 && g - Math.max(r, b) >= 28;
  function legendColors(documentRef, keywords) {
    const result = [];
    for (const element of documentRef.querySelectorAll('button, label, li, span, div')) {
  ...
  function scanCanvas(canvas, keywords) {
    ...
    try {
      ...
      const legends = legendColors(canvas.ownerDocument, keywords);
      ...
      const matchAt = (x, y) => {
        ...
        const legend = legends.find((item) => distance(pixel, item.color) <= 42);
        if (legend) return { keyword: legend.keyword, type: 'legend', color: pixel, availableColor: isAvailableGreen(pixel) ? 'green' : null };
        return isAvailableGreen(pixel) ? { keyword: '绿色可售座位', type: 'green', color: pixel } : null;
      };
      for (let y = 0; y < canvas.height; y += 2) for (let x = 0; x < canvas.width; x += 2) {
        const index = y * canvas.width + x;
        if (visited[index]) continue;
        const match = matchAt(x, y);
        if (!match) continue;
        ...
        while (stack.length && pixels.length < 3000) {
          ...
          for (const [nx, ny] of [[px + 2, py], [px - 2, py], [px, py + 2], [px, py - 2]]) {
            ...
            const nextMatch = matchAt(nx, ny);
            if (!visited[next] && nextMatch && nextMatch.type === match.type && (match.type !== 'legend' || nextMatch.keyword === match.keyword)) { visited[next] = 1; stack.push([nx, ny]); }
          }
        }
        ...
        candidates.push({ canvas, keyword: match.keyword, availableColor: match.availableColor || (match.type === 'green' ? 'green' : null), color: `rgb(${match.color.join(', ')})`, x: center[0], y: center[1], clientX: rect.left + center[0] * scaleX, clientY: rect.top + center[1] * scaleY, pixels: pixels.length, iframe: canvas.ownerDocument.location?.href || null, source: 'canvas-color-region' });
      }
  ...
  function scan(keywords) {
    const documents = globalThis.STAInterparkAdapter?.documents?.().filter((item) => item.document).map((item) => item.document) || [document];
    const reports = documents.flatMap((documentRef) => [...documentRef.querySelectorAll('canvas')].map((canvas) => scanCanvas(canvas, keywords)));
    return { candidates: reports.flatMap((report) => report.candidates), reports };
  }
  function click(candidate) {
    ...
    return { ok: true, label: candidate.keyword, coordinate: { x: Math.round(candidate.x), y: Math.round(candidate.y) }, clientCoordinate: { x: Math.round(candidate.clientX), y: Math.round(candidate.clientY) }, iframe: candidate.iframe };
  }
