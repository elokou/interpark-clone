...
const inputIds = ['eventName', 'seatAreas', 'requireContiguous', 'priorityAreas', 'fallbackAreas', 'excludedAreas', 'quantity', 'rowStart', 'rowEnd', 'priceMin', 'priceMax', 'targetX', 'targetY', 'waitTime', 'scanInterval', 'autoClick', 'autoRotate', 'autoSelectSeats', 'maxCycles', 'testMode', 'debugMode'];

async function load() {
  const stored = await chrome.storage.local.get(KEY);
  state = { ...globalThis.STA_DEFAULTS, ...(stored[KEY] || {}) };
  $('eventName').value = state.eventName;
  $('seatAreas').value = state.seatAreas;
  $('requireContiguous').checked = state.requireContiguous;
  document.querySelectorAll('.ticket-grade').forEach((input) => { input.checked = state.ticketGrades.includes(input.value); });
  document.querySelectorAll('.seat-floor').forEach((input) => { input.checked = state.seatFloors.includes(input.value); });
  ...
  $('targetX').value = state.targetX ?? '';
  $('targetY').value = state.targetY ?? '';
...
async function save() {
  state = {
    ...
    seatAreas: $('seatAreas').value.trim(),
    requireContiguous: $('requireContiguous').checked,
    ticketGrades: [...document.querySelectorAll('.ticket-grade:checked')].map((input) => input.value),
    seatFloors: [...document.querySelectorAll('.seat-floor:checked')].map((input) => input.value),
    ...
    targetX: $('targetX').value === '' ? null : Number($('targetX').value),
    targetY: $('targetY').value === '' ? null : Number($('targetY').value),
...
function updateStatus(status = {}) {
  ...
  $('currentArea').textContent = status.rotation?.currentArea || '—';
...
document.querySelectorAll('.ticket-grade, .seat-floor').forEach((input) => input.addEventListener('change', save));
